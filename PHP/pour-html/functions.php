<?php
	...
	function createInput($title, $name, $value, $errorMessage){
        $xhtml = sprintf('<div class="row">
							<p>%s</p>
							%s
							<input type="text" name="%s" value="%s">
							</div>',$title, $errorMessage, $name, $value);
        return $xhtml;
    } 


	function createText($title, $nameTitle, $errorDescription, $valueDescription){
        $xhtmlOption = sprintf('<div class="row">
							<p>%s</p>
							%s
							<textarea name="%s" rows="5" cols="100">%s</textarea>
							</div>',$title, $errorDescription, $nameTitle, $valueDescription);
        return $xhtmlOption;
    } 
