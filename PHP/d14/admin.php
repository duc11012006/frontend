<?php
    ...
    if (isset($_SESSION['userLogin']) == false) {
        ...
        if (isset($_POST['time_login'])) {
            $newTimeLogin = $_POST['time_login'];
            if ($newTimeLogin < 10) {
                $message = '<p style="color: red">Không được lưu thời gian dưới 10 giây</p>';
            } else {
                file_put_contents(DIR_DATA . 'timeLogin.txt',  $newTimeLogin);
                $message = '<p style="color: green">Cap nhat thoi gian thanh cong</p>';
            }
        }
        ...
        $time = file_get_contents(DIR_DATA . 'timeLogin.txt', true);
        if ($timeCurrent - $timeLogin > $time) {
    ...
    $title = "<h3>Xin chào: {$_SESSION['userLogin']['user_info']['name']}</h3>";
    ?>
    ...
    <div class="admin-dashboard">
        ...
        <div class="main-content">
            ...
            <form action="" method="post">
                <?php echo $message; ?>
                <input name="time_login" type="number" value="<?php echo $time ?>">
                <button>Save</button>
            </form>
        </div>
    </div>
